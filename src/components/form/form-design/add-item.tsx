...
const AddItem: React.FC<ComboBoxProps> = ({ options, onSelect, name }) => {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="md:w-[280px] w-full p-0">
        <Command>
          <CommandInput placeholder={t('questionDesign.searchQuestionType')} />
          <CommandList>
            <CommandEmpty>{t('questionDesign.noQuestionTypeFound')}</CommandEmpty>
            <CommandGroup>
              {options.map((option) => (
                <CommandItem
                  key={option.value}
